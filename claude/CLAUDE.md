...
```bash
/casa/dev/claude-code/.venv/bin/python
```
...
**Sandbox limitation:** Claude Code cannot run `uv` commands due to a sandbox conflict with `.git` marker files in the uv cache ([anthropics/claude-code#22620](https://github.com/anthropics/claude-code/issues/22620)). If a package is missing, ask the user to run `uv add <package> --project /casa/dev/claude-code`.

For more information see `/casa/dev/claude-code/README.md`.
